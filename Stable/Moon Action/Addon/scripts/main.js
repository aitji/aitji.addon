...
system.runInterval(() => {
    world.getAllPlayers().forEach(plr => {
        ...
        if (plr.isSleeping) plr.addTag("action:sleeping")
        else plr.removeTag("action:sleeping")
        if (plr.isEmoting) plr.addTag("action:emoting")
        else plr.removeTag("action:emoting")
