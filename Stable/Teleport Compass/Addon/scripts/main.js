...
world.beforeEvents.itemUse.subscribe(data => {
    ...
    if (item.typeId === "minecraft:recovery_compass" && pl.hasTag(`Admin`)) teleport(pl);
})
...
function teleport(pl) {
    system.run(() => {
        ...
        players.map(function (player) {
            ...
            if (randoms == 0) form.button(`${player.name}\n§8Click Here`, "textures/ui/icon_alex")
            if (randoms == 1) form.button(`${player.name}\n§8Click Here`, "textures/ui/icon_steve")
