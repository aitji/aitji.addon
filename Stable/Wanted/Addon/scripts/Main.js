...
function wanted(pl) {
    mc.system.run(() => {
        const score = getScore('wanted', pl, true) || 0
        const obj = World.getAllPlayers().map(plr => ({ name: plr.name, score: getScore('wanted', plr, true) || 0 }))
        const sort = obj.sort((a, b) => b.score - a.score)
        ...
        form.body(`${Body.join("§r\n§r")
            .replace(/%leaderboard%/g, leaderboard)
            .replace(/%pl\.name%/g, `${place}. ${pl.name}`)
            .replace(/%pl\.score%/g, `${score}`)}`)
