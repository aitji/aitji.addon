...
import { world, system, ScoreboardIdentityType, Scoreboard, Player, ItemStack } from "@minecraft/server";
...
world.beforeEvents.itemUse.subscribe((data) => {
    ...
    const item = data.itemStack as ItemStack
...
world.beforeEvents.chatSend.subscribe((data) => {
    ...
    world.getAllPlayers().map(plr => {
        plrchatroom = getScore('chatroom', plr, true) ?? 0
        if (plr.hasTag(kingTag) && adminSeeAll) {
            plr.sendMessage(`§f[{${chatroom}}] <${pl.name}>§f ${msg}§r`);
            return
        }
        if (plrchatroom === chatroom) {
            plr.sendMessage(`BEBUGGER §f<${pl.name}>§f ${msg}§r`);
            return
        }
    });
});
