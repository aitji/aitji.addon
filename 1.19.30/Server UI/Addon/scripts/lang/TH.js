...
function withdraw(pl)	{
	...
	if (bank === 0){
		...
		pl.tell(`§7> §fขอโทษด้วยคุณไม่มี เงินใน§cบัชชีธนาคาร§fเลย.. §7<`)
	}else{
		pl.runCommand(`playsound random.pop @s ~~~ 50`);
		...
		form.show(pl).then((res) => {
			...
			pl.tell(`§7> §fคุณได้ §aถอนเงิน §fจำนวน §c${res.formValues[0]} §7<`)
		})
	}
}
...
function deposit(pl)	{
	...
	if (money === 0){
		pl.runCommand(`playsound random.break @s ~~~ 50`);
		pl.tell(`§7> §fขอโทษด้วยคุณ§cไม่มี§fเงินเลยสักบาทเดียว §7<`)
	}else{
		pl.runCommand(`playsound random.pop @s ~~~ 50`);
		let form = new ModalFormData();
		form.title(`§e»§c ${pl.name}'s §fInterface §e«`)
		...
		form.show(pl).then((res) => {
			...
			pl.tell(`§7> §fคุณได้ทำการ §cฝากเงิน §fจำนวน §c${res.formValues[0]} §7<`)
		})
	}
}
